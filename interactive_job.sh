...
ram=15
gpu=0
read -p "Enter the partition name (gpu, workq, klab-cpu, klab-gpu, klab-l40s, default $part): " PARTITION 
...
if [[ $PARTITION == "klab-l40s" || $PARTITION == "klab-gpu" ]]
then
...
if [[ $PARTITION != "workq"  &&  $PARTITION != "gpu" && $PARTITION != "klab-cpu" && $PARTITION != "klab-l40s" && $PARTITION != "klab-gpu" ]]
then
